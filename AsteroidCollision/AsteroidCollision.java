...
public class  AsteroidCollision {
    public static int[] asteroidCollision(int[] asteroids) {
        ...
        for (int a : asteroids) {
            ...
            // перевіряємо колізії тільки коли поточний летить вліво
            while (!stack.isEmpty() && a < 0 && stack.peek() > 0) {
                int top = stack.peek();

                if (Math.abs(a) > top) {
                    stack.pop(); // вибухає правий
                    continue;    // перевіряємо наступний
                } else if (Math.abs(a) == top) {
                    stack.pop(); // обидва вибухають
                }
                alive = false; // поточний вибухає
                break;
            }
